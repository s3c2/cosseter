    ...
    switch (currentNode.type) {
                ...
                for (param of currentNode.params) {
                    function addParam(param){
                        ...
                        if (param?.type == 'Identifier') { // no default value
                            ...
                            console.log(`  Warning: ${param?.type || "TYPE MISSING OR PARAM NULL"} as a function parameter is not supported. Generating dummy nodes.`);
                            nodeIdCounter++;
                            relsStream.push([vParameterId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                            nodes[nodeIdCounter] = {
                                label: 'AST_V',
        ...
        case 'ClassDeclaration':
            ...
            if (outputStyle == 'c') {
                ...
                // extends/superClass
                if (currentNode.superClass) {
                    // This doesn't work right. Needs reworked to capture the stuff below.
                    // Next problem is that this print statements don't work either
                    // relsStream.push("GREG LOOK HERE --->")
                    // relsStream.push(currentNode.superClass)
                    // relsStream.push("<--- GREG LOOK HERE")
                    nodeIdCounter++;
                    let vAstNameId = nodeIdCounter;
                    relsStream.push([currentId, vAstNameId, parentOf].join(delimiter) + '\n');
        ...
        case 'ForOfStatement':
            if (outputStyle == 'c') {
                ...
                if (currentNode.body && currentNode.body.type == 'BlockStatement'){
                    ...
                    nodeIdCounter++;
                    relsStream.push([vAstBodyId, nodeIdCounter, parentOf].join(delimiter) + '\n');
...
function analyze(filePath, parentNodeId) {
    ...
    if (filename.endsWith('.json')) {
        console.log(("Skipping " + filename + " because of json").green.inverse);
        return;
    }
    ...
    try {
        ...
        */
        /* HERE Replacement when ready */
        /*
        var root = esprima.parse(sourceCode, {
            range: true,
            loc: true,
            tokens: true,
            comment: true,
        ...
        */
        ...
        console.log(e);
        // console.log("GREG -> ESPRIMA ERROR");
        process.exit(1)
        // fs.writeFileSync('esprimaError.log', "Error in " + filename + "---\n");
    }
    if (!stdoutMode){
        console.dir(root);
    }
