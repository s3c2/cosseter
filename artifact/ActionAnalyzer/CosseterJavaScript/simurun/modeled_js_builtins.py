...
# Weird way to get current file for each process
curr = 0
try:
    for i in range(8000):
        with open(f'builtinLog{curr}.log', "r") as f:
            pass
        curr += 1
except:
    pass

# logger = create_logger("builtin_logger", output_type="file", file_name=f'builtinLog{curr}.log')

logger = create_logger("main_logger", output_type="file")
ncc_replacement = None
nccPass1Replacement = None
nccPass2Replacement = None

def recAddObjs(G, obj):
    if G.get_node_attr(obj).get("type", "") == "function":
        G.allowedFuncs.add(obj)
    for other in  G.get_prop_obj_nodes(obj, exclude_proto=False):
        if not other in G.allowedFuncs:
            recAddObjs(G, other)

def setup_js_builtins(G: Graph):
    global ncc_replacement, nccPass1Replacement, nccPass2Replacement
    ...
    tainted_getInput = G.add_blank_func_to_scope("source_hqbpillvul_ga", scope=G.BASE_SCOPE, python_func=tainted_getInput_constructor)
    G.builtin_constructors.append(tainted_getInput)
    # ncc_replacement = G.add_blank_func_to_scope("__nccwpck_require__", scope=G.BASE_SCOPE, python_func=opgen.handle_ncc_require)
    # G.builtin_constructors.append(ncc_replacement)
    
    # for obj in G.builtin_constructors:
    #     # print(obj)
    #     G.allowedFuncs.add(obj)
    #     for other in  G.get_prop_obj_nodes(obj):
    #         G.allowedFuncs.add(other)
    for obj in G.builtin_constructors:
        recAddObjs(G, obj)
    
    G.typeScriptReplacements["__importStar"] = G.add_blank_func_to_scope("__importStar", scope=G.BASE_SCOPE, python_func=replacement__importStar)
    G.typeScriptReplacements["__importDefault"] = G.add_blank_func_to_scope("__importDefault", scope=G.BASE_SCOPE, python_func=replacement__importDefault)
    G.typeScriptReplacements["__export"] = G.add_blank_func_to_scope("__export", scope=G.BASE_SCOPE, python_func=replacement__export)
    G.typeScriptReplacements["__toCommonJS"] = G.add_blank_func_to_scope("__toCommonJS", scope=G.BASE_SCOPE, python_func=replacement__toCommonJS)
    G.typeScriptReplacements["__commonJS"] = G.add_blank_func_to_scope("__commonJS", scope=G.BASE_SCOPE, python_func=replacement__toCommonJS)
    G.typeScriptReplacements["__awaiter"] = G.add_blank_func_to_scope("__awaiter", scope=G.BASE_SCOPE, python_func=replacement__awaiter)
    
    # G.add_blank_func_to_scope("__importStar", scope=G.BASE_SCOPE, python_func=replacement__importStar)
    nccPass1Replacement = G.add_blank_func_with_og_nodes("REPLACEMENT_NCC_PASS1", python_func=replacement__ncc_require_pass1)
    nccPass2Replacement = G.add_blank_func_with_og_nodes("REPLACEMENT_NCC_PASS2", python_func=replacement__ncc_require_pass2)


def replacement__ncc_require_pass2(G: Graph, call_ast, extra, _, *args):
    logger.debug("\n\n\n\n\nInside of the replaced NCC REQUIRE PASS 2!!")
    logger.debug(f'Call AST: {call_ast}')
    ...
    if not curFilePath in G.packIdToCachePass2:
        G.packIdToCachePass2[curFilePath] = {}

    packId = None
    if len(args) != 1:
        logger.debug("ERROR: Too many (or little) arguements for the replaced require")
        return NodeHandleResult()
    else:
        try:
            # For every require that I have seen (and can handle) it is a direct value passed
            packId = args[0].values[0]

            # For some reason its evaluated as a float even though its a string of an int
            # Although sometimes I have seen it as a string
            packId = str(int(packId))
        except:
            if len(args[0].values) == 0:
                logger.debug("ERROR: Could not extract value from args")
                return NodeHandleResult()

    logger.debug(f'PackID: {packId}')
    if packId and packId in G.packIdToFunc[curFilePath]:
        func_ast = G.packIdToFunc[curFilePath][packId]
    else:
        logger.debug("ERROR: Pack id not in extracted ids")
        return NodeHandleResult()
    
    # If already taken care of (includes circular fix)
    if packId in G.packIdToCachePass2[curFilePath]:
        logger.debug(f'{G.packIdToCachePass2[curFilePath][packId]}')
        return NodeHandleResult(obj_nodes=G.packIdToCachePass2[curFilePath][packId])
    
    # Better way
    # Return the pre-created object to be passed as args (or already passed)
    # NOTE If the arguments is only pass by reference, this wont work :(
    if curFilePath in G.packAstToArgs and func_ast in G.packAstToArgs[curFilePath]:
        oldArgs = G.packAstToArgs[curFilePath][func_ast]
        # We create this ourselves so no need to check
    else:
        logger.debug(f'ERROR: Could not get the args for the func_ast: {func_ast}')
        return NodeHandleResult()
    ...
    if func_obj != "?":
        if not G.mainPack:
            try:
                ...
                if not entry_node in G.possible_cf_nodes:
                    G.mainPack = "NO_MAIN_PACK"
            except Exception as e:
                print("Error getting pack entry:")
                print(e)
                print("---")
        if G.mainPack:
            fakeRequire = G.add_obj_node()
            G.set_node_attr(fakeRequire, ("SKIPPED_REQUIRE", True))
            G.set_node_attr(fakeRequire, ("Fobj", func_obj))
            G.set_node_attr(fakeRequire, ("CAST", call_ast))
            G.set_node_attr(fakeRequire, ("Args", oldArgs))
            G.set_node_attr(fakeRequire, ("PackID", packId))
            G.set_node_attr(fakeRequire, ("FS", G.cur_scope))
            G.set_node_attr(fakeRequire, ("FSTMT", G.cur_stmt))
            G.set_node_attr(fakeRequire, ("FOBJS", G.cur_objs))
            G.set_node_attr(fakeRequire, ("FilePath", curFilePath))
            logger.debug(f'Skipping PACKED require for {packId} for now')
            return NodeHandleResult(obj_nodes=[fakeRequire])
        else:
            G.mainPack = packId
            opgen.call_func_obj(G, func_obj, oldArgs, call_ast=call_ast, func_name=packId, is_new=True, dont_skip=True)
            moduleObj = oldArgs[0].obj_nodes[0]
            exportObjs = G.get_prop_obj_nodes(parent_obj=moduleObj, prop_name='exports')

            retResult = NodeHandleResult(obj_nodes=exportObjs)
            logger.debug(f'{retResult}')
            logger.debug("\n\n\n\n\n")
            return retResult
    
    logger.debug("ERROR: Could not find func object")
    return NodeHandleResult()


def replacement__ncc_require_pass1(G: Graph, call_ast, extra, _, *args):
    logger.debug("\n\n\n\n\nInside of the replaced NCC REQUIRE PASS 1!!")
    logger.debug(f'Call AST: {call_ast}')
    logger.debug(f'Extra: {extra}')
    logger.debug(f'Args: {args}')
    ...
    # Old way
    # Create a fake returned module object
    # retObj = G.add_obj_node()
    # G.set_node_attr(retObj, ("REQUIRE_FROM", packId))
    ...
    cached = False
    if G.packIdToCache[curFilePath].get(func_ast, None) is not None:
        cached = True
        # G.packIdToCache[curFilePath].pop(func_ast)

    if func_obj != "?":
        G.file_stack.append(packId)
        previous_file_path = G.cur_file_path
        G.cur_file_path = packId
        if not cached:
            new_vector = (func_obj, packId, None, oldArgs, None, packId)
            if not packId in G.packStack:
                G.stack2.append(new_vector)
                G.packStack.append(packId)
                opgen.empty_rough_stacks(G, "PACK_REQUIRE")
                G.packStack.pop(-1)
            else:
                # Looping dependency
                cached = True
    ...
    # Moved into call func above
    # caller_cpg = G.find_nearest_upper_CPG_node(call_ast)
    # caller_func = opgen.find_function(G, call_ast)
    # if caller_func is not None:
    #     G.add_edge_if_not_exist(
    #         caller_func, func_ast, {"type:TYPE": "CALLS", "promise_related": False})
    # else:
    ...
    if not cached and G.packIdToCache[curFilePath].get(func_ast):
        G.packIdToCache[curFilePath].pop(func_ast)

    if not cached:
        G.stack1.append((func_obj, packId, None, oldArgs, None, packId))
        opgen.empty_rough_stacks(G, 'pack_finished')

    # Might need something like this come back to me
    # if G.get_node_attr(exportObjs[0]).get('unresolved') == True:
    #     print("UNRESOLVED PACKED OBJECTS!!")
    #     opgen.empty_rough_stacks(G, 'exports_unresolved_packed')
...
def replacement__importStar(G: Graph, call_ast, extra, _, *args):
    logger.debug("\n\n\n\n\nInside of the replaced import star!!")
    logger.debug(f'Call AST: {call_ast}')
    logger.debug(f'Extra: {extra}')
    logger.debug(f'Args: {args}')
    logger.debug("\n\n\n\n\n")
    if len(args) != 0:
        moduleObjs = args[0].obj_nodes
        for obj in moduleObjs:
            if G.get_node_attr(obj).get("SKIPPED_REQUIRE"):
                nn = G.get_prop_name_node("default", obj)
                if nn is None:
                    requireExport = G.add_obj_node()
                    G.set_node_attr(requireExport, ("HOW_TO_HANDLE_REQUIRE_OBJ", obj))
                    G.set_node_attr(requireExport, ("RESOLVER_PROP_NAME", "default"))
                    packId = G.get_node_attr(obj).get("PackID", None)
                    if packId is not None:
                        G.set_node_attr(requireExport, ("PACKED", True))

                    G.add_obj_as_prop(prop_name="default", parent_obj=obj, tobe_added_obj=requireExport)
        return args[0]
    else:
        return NodeHandleResult()


def replacement__importDefault(G: Graph, call_ast, extra, _, *args):
    logger.debug("\n\n\n\n\nInside of the replaced import star!!")
    logger.debug(f'Call AST: {call_ast}')
    logger.debug(f'Extra: {extra}')
    logger.debug(f'Args: {args}')
    ...
    if len(args) != 0:
        ...
        for obj in moduleObjs:
            if G.get_node_attr(obj).get("SKIPPED_REQUIRE"):
                G.set_node_attr(obj, ("ADD_DEFAULT", True))
                nn = G.get_prop_name_node("default", obj)
                if nn is None:
                    requireExport = G.add_obj_node()
                    G.set_node_attr(requireExport, ("HOW_TO_HANDLE_REQUIRE_OBJ", obj))
...
def replacement__export(G: Graph, call_ast, extra, _, *args):
    logger.debug("\n\n\n\n\nInside of the replaced exporter!!")
    logger.debug(f'Call AST: {call_ast}')
    logger.debug(f'Extra: {extra}')
    logger.debug(f'Args: {args}')
    for arg in args:
        ...
        for obj in objs:
            fAstNode = obj
            logger.debug(f'{fAstNode}')
            logger.debug(f'{G.get_node_attr(fAstNode)}')
            logger.debug("-")
        logger.debug("---")
    logger.debug("\n\n\n\n\n")

    if len(args) == 2:
        newExportsObj = args[0].obj_nodes[0] 
        exportsToAddObj = args[1].obj_nodes[0]

        for e in G.get_out_edges(exportsToAddObj, edge_type='OBJ_TO_PROP'):
            nameNode = e[1]
            name = G.get_node_attr(nameNode).get("name", None)
            logger.debug(f'{nameNode}')
            logger.debug(f'{name}')
            if name and not name in ['__proto__', 'prototype', 'constructor']:
                logger.debug(f'{name} is not basic!')
                exportFunc = G.get_objs_by_name_node(nameNode)
                logger.debug(f'Export functions: {exportFunc}')
                if len(exportFunc) > 0 and G.get_node_attr(exportFunc[0]).get("type") == "function":
                    logger.debug("Before running the call func")
                    returned_objects, created_objects = opgen.call_func_obj(G, exportFunc[0])[:2]
                    keepObjs = set()
                    logger.debug("After running the call func")
                    logger.debug(f'Export that would be added: {name}')
                    logger.debug("Returned objects:")
                    for obj in returned_objects:
                        logger.debug(f'{G.get_node_attr(obj)}')
                        logger.debug("-")
                        keepObjs.add(obj)
                    ...
                    if len(keepObjs) > 1:
                        logger.debug("POTENTIAL ERROR: MULTIPLE KEEP OBJS IN __EXPORT")

                    if len(keepObjs) > 0:
                        G.add_obj_as_prop(prop_name=name, parent_obj=newExportsObj, tobe_added_obj=list(keepObjs)[0])
                    else:
                        print("EXPORT NOT FOUND FOR CALL_AST [%s]" % call_ast)
...
def replacement__toCommonJS(G: Graph, call_ast, extra, _, *args):
    logger.debug("\n\n\n\n\nInside of the replaced toCommonJs!!")
    logger.debug(f'Call AST: {call_ast}')
    logger.debug(f'Extra: {extra}')
    logger.debug(f'Args: {args}')
    logger.debug("\n\n\n\n\n")
...
def replacement__awaiter(G: Graph, call_ast, extra, _, *args):
    logger.debug("\n\n\n\n\nInside of the replaced awaiter!!")
    logger.debug(f'Call AST: {call_ast}')
    logger.debug(f'Extra: {extra}')
    logger.debug(f'Args: {args}')
    logger.debug("\n\n\n\n\n")
    if len(args) != 0:
        cfp = G.get_cur_file_path()
        runFuncObjs = args[-1].obj_nodes
        logger.debug(f'anon func to run: {runFuncObjs}')
        result, _ = opgen.call_function(G, runFuncObjs, call_ast=call_ast, extra=extra)
        # This could break things...
        promiseObject = G.add_obj_node()
        G.set_node_attr(promiseObject, ("fulfilled_with", result))
        return NodeHandleResult(obj_nodes=[promiseObject])
        logger.debug("Result for handle awaiter:")
        logger.debug(f'{result}')
        term = None
        try:
            term = result.terminated
        except:
            logger.debug(f'Functions {(str(runFuncObjs))} DID NOT terminate')
        
        if term is not None:
            logger.debug(f'Functions {(str(runFuncObjs))} terminated')

        if result is not None:
            return result

    logger.debug("Made it to the end of awaiter!!!")
    return NodeHandleResult()


def tainted_getInput_constructor(G: Graph, call_ast, extra, _, strs):
    """
    returned_obj = G.add_obj_node(call_ast)
    G.set_node_attr(returned_obj, ('tainted', True))
    # G.set_node_attr(returned_obj, ('getInput', True))
    # G.set_node_attr(returned_obj, ('fake_arg', True))
    # G.set_node_attr(returned_obj, ('code', wildcard))
    # args, src, _ = to_values(G, args, call_ast)
    # return NodeHandleResult(used_objs=[returned_obj], obj_nodes=[returned_obj], values=[wildcard], value_sources=[[returned_obj]])
    return NodeHandleResult(obj_nodes=[returned_obj])
    """
    returned_obj = G.add_obj_node(call_ast)
    G.set_node_attr(returned_obj, ('tainted', "GET_INPUT_TAINT"))
    values, sources, _ = to_values(G, strs, call_ast)
    returned_values = []
    for s in values:
        returned_values.append(wildcard)
    used_objs = list(filter(lambda x: x != wildcard, chain(*sources)))
    logger.debug('GREG LOOK HERE: [%s]'.format(returned_values))
    return NodeHandleResult(obj_nodes=[returned_obj], values=returned_values, value_sources=sources,
        used_objs=used_objs)
...
def setup_object_and_function(G: Graph):
    ...
    # Githubs messy functions
    G.add_blank_func_as_prop('defineProperty', object_cons, object_define_property)
    G.add_blank_func_as_prop('defineProperties', object_cons, blank_func)
    G.add_blank_func_as_prop('getOwnPropertyDescriptor', object_cons, blank_func)
    G.add_blank_func_as_prop('getOwnPropertyNames', object_cons, object_get_property_names)
    # Might not need this for now
    # G.add_blank_func_as_prop('getPrototypeOf', object_cons, object_get_prototype_of)
    

#                                                       V (_) is itself so [->Object<-].defineProperty in case I need it later
def object_define_property(G: Graph, caller_ast, extra, _, target, prop, descriptor):
    # PLEASE NOTE!! This is incomplete and only handles 'get' since it is the only one used by github!!
    # enumerable is also used, but it is always set to true and fast doesn't have the functionality to ignore it if it was false anyway
    logger.log(ATTENTION, f'GREG -> [S] Started the [defineProperty] function')
    logger.log(ATTENTION, f'GREG -> Caller ast [{caller_ast}]\n---')
    logger.log(ATTENTION, f'GREG -> Extra [{extra}]\n---')
    logger.log(ATTENTION, f'GREG -> Target Object [{target}]\n---')
    logger.log(ATTENTION, f'GREG -> Property Name [{prop}]\n---')
    logger.log(ATTENTION, f'GREG -> Descriptor [{descriptor}]\n---')
    try:
        descriptorObject = descriptor.obj_nodes[0]
        logger.log(ATTENTION, f'GREG -> Descriptor out edges[{G.get_out_edges(descriptorObject, edge_type="OBJ_TO_PROP")}]\n---')
        for e in G.get_out_edges(descriptorObject, edge_type="OBJ_TO_PROP"):
            logger.log(ATTENTION, f'GREG -> Current edge attributes[{G.get_node_attr(e[1])}]\n---')
            if G.get_node_attr(e[1]).get('name', "") == "get":
                logger.log(ATTENTION, f'GREG -> "Get" out edges[{G.get_out_edges(e[1])}]\n---')
                for a in G.get_out_edges(e[1]):
                    if G.get_node_attr(a[1]).get('type', "") == "function":
                        logger.log(ATTENTION, f'GREG -> Current "Get" edge attributes[{G.get_node_attr(a[1])}]\n---')
                        returned_objects, created_objects = opgen.call_func_obj(G, a[1])[:2]
                        # fResult, created_objects = opgen.call_function(G, a[1], extra=extra, call_ast=caller_ast)
                        # returned_objects = fResult.obj_nodes
      
                        propertyObjects = []
                        propertyObjects.extend(returned_objects)
                        logger.log(ATTENTION, f'GREG -> returned_objects [{returned_objects}]')
                        logger.log(ATTENTION, f'GREG -> created_objects [{created_objects}]')
                        logger.log(ATTENTION, f'GREG -> propertyObjects list [{propertyObjects}]')
                        propName = G.get_node_attr(prop.obj_nodes[0]).get('code', None)
                        if len(propertyObjects) > 1:
                            found = False
                            for obj in propertyObjects:
                                if G.get_node_attr(obj).get("name", "") == propName:
                                    propertyObjects = obj
                                    found = True
                                    break
                            if not found:
                                logger.log(ATTENTION, f'WARNING: More than one object found in defineProperty and no clear one was chosen!')
                                return NodeHandleResult()
                        else:
                            propertyObjects = propertyObjects[0]
                        logger.log(ATTENTION, f'GREG -> propertyObjects after filter [{propertyObjects}]')
                        targetObj = target.obj_nodes[0]
                        if propName:
                            G.add_obj_as_prop(prop_name=propName, parent_obj=targetObj, tobe_added_obj=propertyObjects)
                            logger.log(ATTENTION, f'GREG -> [E] Ending the [defineProperty] function')
                            # return NodeHandleResult(obj_nodes=[target])
                            return target
            if G.get_node_attr(e[1]).get('name', "") == "value":
                logger.log(ATTENTION, f'GREG -> "Value" out edges[{G.get_out_edges(e[1])}]\n---')
                for a in G.get_out_edges(e[1]):
                    logger.log(ATTENTION, f'GREG -> Current "Value" edge attributes[{G.get_node_attr(a[1])}]\n---')
                    propertyObjects = a[1]
                    logger.log(ATTENTION, f'GREG -> returned object which should be the property [{propertyObjects}]')
                    propName = None
                    if len(prop.values) > 0:
                        propName = prop.values[0]
                    else:
                        vals, sources, _ = opgen.to_values(G, prop, for_prop=True)
                        propName = vals[0]
                    targetObj = target.obj_nodes[0]
        ...
        logger.log(ATTENTION, f'GREG -> ERROR: {e}')
        logger.log(ATTENTION, f'GREG -> [E] Ending the [defineProperty] function')
    return NodeHandleResult()


def object_get_property_names(G: Graph, caller_ast, extra, _, target):
    # PLEASE NOTE!! This is incomplete and only handles 'get' since it is the only one used by github!!
    # enumerable is also used, but it is always set to true and fast doesn't have the functionality to ignore it if it was false anyway
    logger.log(ATTENTION, f'GREG -> [S] Started the [getOwnPropertyNames] function')
    logger.log(ATTENTION, f'GREG -> Caller ast [{caller_ast}]\n---')
    logger.log(ATTENTION, f'GREG -> Extra [{extra}]\n---')
    logger.log(ATTENTION, f'GREG -> Target Object [{target}]\n---')
    try:
        targetObject = target.obj_nodes[0]
        logger.log(ATTENTION, f'GREG -> Target out edges (properties) [{G.get_out_edges(targetObject, edge_type="OBJ_TO_PROP")}]\n---')
        lst = []
        for e in G.get_out_edges(targetObject, edge_type="OBJ_TO_PROP"):
            attr = G.get_node_attr(e[1])
            logger.log(ATTENTION, f'GREG -> Current edge attributes[{attr}]\n---')
            if not attr.get('name', "") in ["", "__proto__", "constructor"]:
                lst.append(attr.get('name'))
            
        arr = G.add_obj_node(caller_ast, 'array')
        G.add_obj_as_prop('length', caller_ast, 'number', value=len(lst),
            parent_obj=arr)
        for j,item in enumerate(lst):
            sObj = G.add_obj_node(js_type="string", value=item)
            G.add_obj_as_prop(prop_name=str(j), parent_obj=arr, tobe_added_obj=sObj)
        logger.log(ATTENTION, f'GREG -> [E] Ending the [getOwnPropertyNames] function')
        return NodeHandleResult(obj_nodes=[arr])
    except Exception as e:
        logger.log(ATTENTION, f'GREG -> ERROR: {e}')
        logger.log(ATTENTION, f'GREG -> [E] Ending the [getOwnPropertyNames] function')
    return NodeHandleResult()
...
def array_p_for_each_value(G: Graph, caller_ast, extra, array=NodeHandleResult(), callback=NodeHandleResult(), this=None):
    ...
    for arr in array.obj_nodes:
        ...
        for name_node in name_nodes:
            ...
            # NOTE This causes an error because the call back is not called before this pop is called
            # Added in this statement to counteract it but it will most likely break some of the debugging
...
def object_is(G: Graph, caller_ast, extra, _, value1: NodeHandleResult, value2: NodeHandleResult):
    val1Vals, _, _ = opgen.to_values(G, value1)
    val2Vals, _, _ = opgen.to_values(G, value2)
    if set(value1.obj_nodes) == set(value2.obj_nodes) and \
            set(val1Vals) == set(val2Vals):
...
def object_p_has_own_property(G: Graph, caller_ast, extra, this, *args):
    logger.log(ATTENTION, f'GREG -> HAS OWN PROP WAS RUN')
...
def function_p_bind(G: Graph, caller_ast, extra, func: NodeHandleResult, this=NodeHandleResult(), *args):
    # print("\n\n\n\n\n")
    # print("Inside bind function!")
    # print("Caller AST:", caller_ast)
    # print("Extra:", extra)
    # print("Func:", func)
    # print("This:", this)
    # print("Args:", args)
    ...
    for f in func.obj_nodes:
        # TODO Potentially fix me as this isn't the way to do it most likely
        # Works without :)
        # if f == opgen.octokitRequire:
        #     return NodeHandleResult(obj_nodes=[f])
...
def setup_global_objs(G: Graph):
    ...
    G.set_node_attr(process_obj, ('PROCESS_OBJ', True))
    ...
    # Here is where I will need to add env most likely
    envObj = G.add_obj_as_prop(prop_name="env", parent_obj=process_obj, js_type="object", value=wildcard)
    G.set_node_attr(envObj, ('fake_arg', True))
    G.set_node_attr(envObj, ('ENV_OBJECT', True))
    art = G.add_obj_as_prop(prop_name='ACTIONS_ID_TOKEN_REQUEST_TOKEN', parent_obj=envObj, js_type='string', value="ACTIONS_ID_TOKEN_REQUEST_TOKEN")
    arl = G.add_obj_as_prop(prop_name='ACTIONS_ID_TOKEN_REQUEST_URL', parent_obj=envObj, js_type='string', value='ACTIONS_ID_TOKEN_REQUEST_URL')
    G.set_node_attr(art, ('tainted', "ACTIONS_ID_TOKEN_REQUEST_TOKEN"))
    G.set_node_attr(arl, ('tainted', "ACTIONS_ID_TOKEN_REQUEST_URL"))
...
def console_log(G: Graph, caller_ast, extra, _, *args):
    ...
    for i, arg in enumerate(args):
        used_objs.update(arg.obj_nodes)
        # used_objs.update(arg.used_objs)
        argVals, _, _ = opgen.to_values(G, arg)
        values = list(map(str, argVals))
        for obj in arg.obj_nodes:
...
def promise_p_then(G: Graph, call_ast, extra, this, on_fulfilled=NodeHandleResult(), on_rejected=NodeHandleResult()):
    ...
    def check_promise_status(self, G):
        ...
        for promise in old_promises:
            ...
            if G.first_pass and fulfilled_with is None:
            ...
            if G.first_pass and rejected_with is None:
    ...
    check_promise_status(check_promise_status, G)
