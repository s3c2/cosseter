...
def esprima_parse(path='-', args=[], input=None, print_func=print):
    ...
    # print_func(stderr)
    l = 10000
    e = str(stderr)[-l:] if len(str(stderr)) > l else str(stderr)
    if proc.returncode != 0:
        raise subprocess.CalledProcessError(proc.returncode, e)

    # with open("/home/gttystah/GitHub_Permission_Reduction/abstractTests/JupyterTestingNotebooks/out.txt", "a") as f:
    #     f.write("START ESPREE PARSE--->\n")
    #     f.write(stdout)
    #     f.write("\nEND ESPREE PARSE<---")
    return stdout
