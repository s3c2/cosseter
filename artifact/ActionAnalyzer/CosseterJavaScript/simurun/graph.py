...
class Graph:
    ...
    def __init__(self):
        # For debugging
        self.getNeededInfo = False
        self.savedNeededInfo = False
        self.funcTimes = {}

        # To capture any lodash functions I may miss
        self.lodashCalls = set()
        self.handleRecursion = {}
        self.skipAddingBackToStack = False
        self.placeholderASTDef = None
        self.packStack = []
        self.depthReached = False
        self.gitCalls = {}
        self.execGitCalls = set()
        
        # Improved network handling
        self.networkFunctions = {"paginate", "request", "fetch", "curl", "get", "post", "put", "patch", "del", "getJson", "postJson", "putJson", "patchJson"}
        self.networkAsts = set()
        self.contextMethodObjs = set()
        self.procInpsToContext = []

        # Outer Control Flow
        self.simple = False
        self.firstPassRequires = set()
        self.handledDemandedFiles = set()

        # Toplevel Timeout
        self.overallStartTime = None
        self.profileTimeout = None

        # Node Traversal
        self.lastNodeInfo = None

        # Mongo Result Updater
        self.updatedOnce = False
        self.updatedLast = False
        self.possibleOctokitMethods = set()
        self.possibleNetworkCalls = set()
        self.missingTypes = set()

        # Dependency Slicing
        self.sliceDependency = False
        self.setSlicer = False
        self.dependencyGraph = {}
        self.importantDependencies = set()
        self.allowedDependencies = set()
        self.allowedDependencies = {5202, '/home/gttystah/GitHub_Permission_Reduction/abstractTests/dependencySlice/everythingTogetherWithDefaultsAndPlugin.js', 4202, 3202}
        self.GithubActions = False
        self.limitDependencies = False
        self.calledAstNodes = {}
        self.filesLookedInto = set()
        self.modules = set()
        self.stackHandledFuncs = set()
        self.typeScriptReplacements = {}
        self.possibleAdditionsCF = set()
        self.curModuleStatement = False
        self.foundImportFuncs = set()
        self.createdOctoMethods = set()
        self.octokitAdditions = set()
        self.octokitUsage = set()
        self.curOctoStmt = False
        self.prePass = False
        self.demandDriven = False
        self.packerAsts = set()
        self.packIdToFunc = {}
        self.packAstToArgs = {}

        # I added this since they aren't looked at twice so it needs to be run as seen
        self.packIdToCache = {}
        self.packIdToCachePass2 = {}
        self.mainPack = None
        ...
        # Error finders
        self.timeoutEver = False
        self.graphQl = False
        self.simpleGit = False
        self.timeoutVals = {}
        self.requireDepth = 0

        self.debugWait = False
        self.notHandled = set()
        self.badRequires = set()
        self.functionDepth = {}
        self.currentNccEntry = "MAIN"
        self.functionCalls = {}
        self.allowedFuncs = set()
        self.id_token = False
        self.graphqlFound = []
        ...
        # My optimization list
        self.func_obj_counts = {}
        self.overFuncObj = set()
        self.cachedNodeResults = {}
        self.nodeUses = {}
        self.nodesRan = 0
        self.argsPassed = {}
        self.esprimaError = False
        self.backup_objs = {}
        self.exp = False
        self.objsUsed = {}
        self.allowedMultiCall = {}
        self.compileRequires = ["__nccwpck_require__", "__webpack_require__"]
        self.currWebpack = {}
        self.moduleCache = {}
        self.restCallAsts = set()
        self.currentNccast = {}
        self.nccKeeps = {}
        self.extraHandle = False
        # {params (string val): [G.last_stmt, G.cur_stmt, returned_objects, used_objects]}
        self.requireCache = {}
        self.nccRequireTimeoutStack = []
        self.currentTimeoutInd = -1
        self.pastTimeout = -1
        self.timeoutConnections = {}
        self.foundPerms = {}
        self.cd_paths = []
        self.octoPresent = False
        self.netPresent = False
        self.execPresent = False
        self.potentialMisses = []
        self.actionFile = "MISSING"
        self.actionVersion = "MISSING"
        self.nccsHandled = {}
        ...
        # Cosseter Stuff
        self.keepFullCode = True

    def copyGraph(self, newGraph):
        # Use the line below as a format
        newGraph.getNeededInfo = copy.deepcopy(self.getNeededInfo)
        newGraph.savedNeededInfo = copy.deepcopy(self.savedNeededInfo)

        # Improved network handling
        newGraph.networkFunctions = copy.deepcopy(self.networkFunctions)
        newGraph.networkAsts = copy.deepcopy(self.networkAsts)

        # Outer Control Flow
        newGraph.simple = copy.deepcopy(self.simple)
        newGraph.firstPassRequires = copy.deepcopy(self.firstPassRequires)
        newGraph.handledDemandedFiles = copy.deepcopy(self.handledDemandedFiles)

        # Toplevel Timeout
        newGraph.overallStartTime = copy.deepcopy(self.overallStartTime)
        newGraph.profileTimeout = copy.deepcopy(self.profileTimeout)

        # Node Traversal
        newGraph.lastNodeInfo = copy.deepcopy(self.lastNodeInfo)

        # Mongo Result Updater
        newGraph.updatedOnce = copy.deepcopy(self.updatedOnce)
        newGraph.updatedLast = copy.deepcopy(self.updatedLast)
        newGraph.possibleOctokitMethods = copy.deepcopy(self.possibleOctokitMethods)
        newGraph.possibleNetworkCalls = copy.deepcopy(self.possibleNetworkCalls)
        newGraph.missingTypes = copy.deepcopy(self.missingTypes)

        # Dependency Slicing
        newGraph.sliceDependency = copy.deepcopy(self.sliceDependency)
        newGraph.setSlicer = copy.deepcopy(self.setSlicer)
        newGraph.dependencyGraph = copy.deepcopy(self.dependencyGraph)
        newGraph.importantDependencies = copy.deepcopy(self.importantDependencies)
        newGraph.allowedDependencies = copy.deepcopy(self.allowedDependencies)
        newGraph.allowedDependencies = copy.deepcopy(self.allowedDependencies)
        newGraph.GithubActions = copy.deepcopy(self.GithubActions)
        newGraph.limitDependencies = copy.deepcopy(self.limitDependencies)
        newGraph.calledAstNodes = copy.deepcopy(self.calledAstNodes)
        newGraph.filesLookedInto = copy.deepcopy(self.filesLookedInto)
        newGraph.modules = copy.deepcopy(self.modules)
        newGraph.stackHandledFuncs = copy.deepcopy(self.stackHandledFuncs)
        newGraph.typeScriptReplacements = copy.deepcopy(self.typeScriptReplacements)
        newGraph.possibleAdditionsCF = copy.deepcopy(self.possibleAdditionsCF)
        newGraph.curModuleStatement = copy.deepcopy(self.curModuleStatement)
        newGraph.foundImportFuncs = copy.deepcopy(self.foundImportFuncs)
        newGraph.createdOctoMethods = copy.deepcopy(self.createdOctoMethods)
        newGraph.octokitAdditions = copy.deepcopy(self.octokitAdditions)
        newGraph.octokitUsage = copy.deepcopy(self.octokitUsage)
        newGraph.curOctoStmt = copy.deepcopy(self.curOctoStmt)
        newGraph.prePass = copy.deepcopy(self.prePass)
        newGraph.demandDriven = copy.deepcopy(self.demandDriven)
        newGraph.packerAsts = copy.deepcopy(self.packerAsts)
        newGraph.packIdToFunc = copy.deepcopy(self.packIdToFunc)
        newGraph.packAstToArgs = copy.deepcopy(self.packAstToArgs)

        # I added this since they aren't looked at twice so it needs to be run as seen
        newGraph.packIdToCache = copy.deepcopy(self.packIdToCache)
        newGraph.packIdToCachePass2 = copy.deepcopy(self.packIdToCachePass2)
        newGraph.mainPack = copy.deepcopy(self.mainPack)

        # Contraint solver
        newGraph.routesFound = copy.deepcopy(self.routesFound)
        newGraph.preCheck = copy.deepcopy(self.preCheck)
        newGraph.hasNcc = copy.deepcopy(self.hasNcc)

        # Error finders
        newGraph.timeoutEver = copy.deepcopy(self.timeoutEver)
        newGraph.graphQl = copy.deepcopy(self.graphQl)
        newGraph.simpleGit = copy.deepcopy(self.simpleGit)
        newGraph.timeoutVals = copy.deepcopy(self.timeoutVals)
        newGraph.requireDepth = copy.deepcopy(self.requireDepth)

        newGraph.debugWait = copy.deepcopy(self.debugWait)
        newGraph.notHandled = copy.deepcopy(self.notHandled)
        newGraph.badRequires = copy.deepcopy(self.badRequires)
        newGraph.functionDepth = copy.deepcopy(self.functionDepth)
        newGraph.currentNccEntry = copy.deepcopy(self.currentNccEntry)
        newGraph.functionCalls = copy.deepcopy(self.functionCalls)
        newGraph.allowedFuncs = copy.deepcopy(self.allowedFuncs)
        newGraph.id_token = copy.deepcopy(self.id_token)
        newGraph.graphqlFound = copy.deepcopy(self.graphqlFound)

        newGraph.functionTimes = copy.deepcopy(self.functionTimes)
        newGraph.cutoffs = copy.deepcopy(self.cutoffs)

        # My optimization list
        newGraph.func_obj_counts = copy.deepcopy(self.func_obj_counts)
        newGraph.overFuncObj = copy.deepcopy(self.overFuncObj)
        newGraph.cachedNodeResults = copy.deepcopy(self.cachedNodeResults)
        newGraph.nodeUses = copy.deepcopy(self.nodeUses)
        newGraph.nodesRan = copy.deepcopy(self.nodesRan)
        newGraph.argsPassed = copy.deepcopy(self.argsPassed)
        newGraph.esprimaError = copy.deepcopy(self.esprimaError)
        newGraph.backup_objs = copy.deepcopy(self.backup_objs)
        newGraph.exp = copy.deepcopy(self.exp)
        newGraph.objsUsed = copy.deepcopy(self.objsUsed)
        newGraph.allowedMultiCall = copy.deepcopy(self.allowedMultiCall)
        newGraph.compileRequires = copy.deepcopy(self.compileRequires)
        newGraph.currWebpack = copy.deepcopy(self.currWebpack)
        newGraph.moduleCache = copy.deepcopy(self.moduleCache)
        newGraph.restCallAsts = copy.deepcopy(self.restCallAsts)
        newGraph.currentNccast = copy.deepcopy(self.currentNccast)
        newGraph.nccKeeps = copy.deepcopy(self.nccKeeps)
        newGraph.extraHandle = copy.deepcopy(self.extraHandle)
        # {params (string val): [G.last_stmt, G.cur_stmt, returned_objects, used_objects]}
        newGraph.requireCache = copy.deepcopy(self.requireCache)
        newGraph.nccRequireTimeoutStack = copy.deepcopy(self.nccRequireTimeoutStack)
        newGraph.currentTimeoutInd = copy.deepcopy(self.currentTimeoutInd)
        newGraph.pastTimeout = copy.deepcopy(self.pastTimeout)
        newGraph.timeoutConnections = copy.deepcopy(self.timeoutConnections)
        newGraph.foundPerms = copy.deepcopy(self.foundPerms)
        newGraph.cd_paths = copy.deepcopy(self.cd_paths)
        newGraph.octoPresent = copy.deepcopy(self.octoPresent)
        newGraph.netPresent = copy.deepcopy(self.netPresent)
        newGraph.execPresent = copy.deepcopy(self.execPresent)
        newGraph.potentialMisses = copy.deepcopy(self.potentialMisses)
        newGraph.actionFile = copy.deepcopy(self.actionFile)
        newGraph.actionVersion = copy.deepcopy(self.actionVersion)
        newGraph.nccsHandled = copy.deepcopy(self.nccsHandled)

        newGraph.exec_counter_limit = copy.deepcopy(self.exec_counter_limit)

        newGraph.graph = copy.deepcopy(self.graph)
        newGraph.cur_objs = copy.deepcopy(self.cur_objs)
        newGraph.cur_scope = copy.deepcopy(self.cur_scope)
        newGraph.cur_func = copy.deepcopy(self.cur_func)
        newGraph.cur_id = copy.deepcopy(self.cur_id)
        newGraph.entry_file_path = copy.deepcopy(self.entry_file_path)
        newGraph.cur_file_path = copy.deepcopy(self.cur_file_path)
        newGraph.file_contents = copy.deepcopy(self.file_contents)
        newGraph.logger = copy.deepcopy(self.logger)
        newGraph.mark_taint_vals = copy.deepcopy(self.mark_taint_vals)
        newGraph.cpg_node_cache = copy.deepcopy(self.cpg_node_cache)

        # for evaluation
        newGraph.covered_num_stat = copy.deepcopy(self.covered_num_stat)
        newGraph.total_num_stat = copy.deepcopy(self.total_num_stat)
        newGraph.covered_stat = copy.deepcopy(self.covered_stat)
        newGraph.all_stat = copy.deepcopy(self.all_stat)
        newGraph.all_func = copy.deepcopy(self.all_func)
        newGraph.covered_func = copy.deepcopy(self.covered_func)

        # exit immediately when vulnerability is found
        newGraph.finished = copy.deepcopy(self.finished)
        newGraph.exit_when_found = copy.deepcopy(self.exit_when_found)
        newGraph.func_start_time = copy.deepcopy(self.func_start_time)
        newGraph.time_limit_reached = copy.deepcopy(self.time_limit_reached)

        # JS internal values
        newGraph.function_prototype = copy.deepcopy(self.function_prototype)
        newGraph.object_prototype = copy.deepcopy(self.object_prototype)
        newGraph.array_prototype = copy.deepcopy(self.array_prototype)
        newGraph.number_prototype = copy.deepcopy(self.number_prototype)
        newGraph.string_prototype = copy.deepcopy(self.string_prototype)
        newGraph.boolean_prototype = copy.deepcopy(self.boolean_prototype)
        newGraph.regexp_prototype = copy.deepcopy(self.regexp_prototype)
        newGraph.promise_prototype = copy.deepcopy(self.promise_prototype)

        newGraph.builtin_constructors = copy.deepcopy(self.builtin_constructors)
        newGraph.builtin_prototypes = copy.deepcopy(self.builtin_prototypes)

        # scope name counter
        # newGraph.scope_counter = copy.deepcopy(self.scope_counter)

        # contains a list of node ids based on the ast id
        newGraph.call_stack = copy.deepcopy(self.call_stack)
        newGraph.for_stack = copy.deepcopy(self.for_stack)
        # newGraph.caller_counter = copy.deepcopy(self.caller_counter)
        newGraph.caller_stack = copy.deepcopy(self.caller_stack)
        newGraph.call_limit = copy.deepcopy(self.call_limit)
        newGraph.file_stack = copy.deepcopy(self.file_stack)
        newGraph.require_obj_stack = copy.deepcopy(self.require_obj_stack)
        newGraph.cur_stmt = copy.deepcopy(self.cur_stmt)
        newGraph.function_returns = copy.deepcopy(self.function_returns)

        # Python-modeled built-in modules
        newGraph.builtin_modules = copy.deepcopy(self.builtin_modules)

        # prototype pollution
        newGraph.check_proto_pollution = copy.deepcopy(self.check_proto_pollution)
        newGraph.proto_pollution = copy.deepcopy(self.proto_pollution)
        newGraph.func_entry_point = copy.deepcopy(self.func_entry_point)

        # internal property tampering
        newGraph.check_ipt = copy.deepcopy(self.check_ipt)
        newGraph.ipt_write = copy.deepcopy(self.ipt_write)
        newGraph.ipt_use = copy.deepcopy(self.ipt_use)

        newGraph.run_all = copy.deepcopy(self.run_all)
        newGraph.no_file_based = copy.deepcopy(self.no_file_based)
        newGraph.call_graph_dist = copy.deepcopy(self.call_graph_dist)
        newGraph.two_pass = copy.deepcopy(self.two_pass)
        newGraph.first_pass = copy.deepcopy(self.first_pass)
        newGraph.coarse_only = copy.deepcopy(self.coarse_only)
        newGraph.cf_guided = copy.deepcopy(self.cf_guided)
        newGraph.finished_num_of_passes = copy.deepcopy(self.finished_num_of_passes)
        newGraph.possible_cf_nodes = copy.deepcopy(self.possible_cf_nodes)
        newGraph.allowed_exported_funcs = copy.deepcopy(self.allowed_exported_funcs)
        newGraph.vul_files = copy.deepcopy(self.vul_files)
        newGraph.function_time_limit = copy.deepcopy(self.function_time_limit)
        newGraph.single_branch = copy.deepcopy(self.single_branch)
        newGraph.print = copy.deepcopy(self.print)
        newGraph.interactive = copy.deepcopy(self.interactive)
        newGraph.output_status = copy.deepcopy(self.output_status)
        newGraph.new_trace_rule = copy.deepcopy(self.new_trace_rule)
        newGraph.cf_searches = copy.deepcopy(self.cf_searches)

        # newGraph.func_call_queue = copy.deepcopy(self.func_call_queue)
        newGraph.cur_func_call = copy.deepcopy(self.cur_func_call)
        newGraph.stack1 = copy.deepcopy(self.stack1)
        newGraph.stack2 = copy.deepcopy(self.stack2)
        newGraph.rerun_counter = copy.deepcopy(self.rerun_counter)
        newGraph.unresolvable_calls = copy.deepcopy(self.unresolvable_calls)
        newGraph.dynamic_calls = copy.deepcopy(self.dynamic_calls)
        newGraph.static_calls = copy.deepcopy(self.static_calls)
        newGraph.total_calls = copy.deepcopy(self.total_calls)
        # newGraph.vul_files_ast = copy.deepcopy(self.vul_files_ast)

        newGraph.vul_type = copy.deepcopy(self.vul_type)

        # source code taint highlighting
        # newGraph.obj_to_ast = copy.deepcopy(self.obj_to_ast)
        newGraph.highlighted_obj_nodes = copy.deepcopy(self.highlighted_obj_nodes)

        # constraint solver
        # newGraph.solve_from = copy.deepcopy(self.solve_from)
        # newGraph.solve_for = copy.deepcopy(self.solve_for)
        # newGraph.solve_mode = copy.deepcopy(self.solve_mode)
        # newGraph.reverse_names = copy.deepcopy(self.reverse_names)
        # newGraph.extra_constraints = copy.deepcopy(self.extra_constraints)
        # newGraph.auto_exploit = copy.deepcopy(self.auto_exploit)
        # newGraph.cur_source_name = copy.deepcopy(self.cur_source_name)

        newGraph.success_detect = copy.deepcopy(self.success_detect)
        newGraph.success_exploit = copy.deepcopy(self.success_exploit)

        # control flow & data flow
        newGraph.last_stmts = copy.deepcopy(self.last_stmts)
        newGraph.num_of_cf_paths = copy.deepcopy(self.num_of_cf_paths)
        newGraph.num_of_prec_cf_paths = copy.deepcopy(self.num_of_prec_cf_paths)
        newGraph.num_of_full_cf_paths = copy.deepcopy(self.num_of_full_cf_paths)
        newGraph.dont_quit = copy.deepcopy(self.dont_quit)
        newGraph.scope_stack = copy.deepcopy(self.scope_stack)

        # task queues
        newGraph.task_queue = copy.deepcopy(self.task_queue)
        newGraph.microtask_queue = copy.deepcopy(self.microtask_queue)

        newGraph.array_obj_node_limit = copy.deepcopy(self.array_obj_node_limit)
        newGraph.op_obj_node_limit = copy.deepcopy(self.op_obj_node_limit)
        newGraph.op_value_limit = copy.deepcopy(self.op_value_limit)

        newGraph.array_obj_node_limit = copy.deepcopy(self.array_obj_node_limit)
        newGraph.op_obj_node_limit = copy.deepcopy(self.op_obj_node_limit)
        newGraph.op_value_limit = copy.deepcopy(self.op_value_limit)

        newGraph.num_of_ast_nodes = copy.deepcopy(self.num_of_ast_nodes)

        csv.field_size_limit(2 ** 31 - 1)
        class joern_dialect(csv.excel_tab):
            doublequote = False
            escapechar = '\\'
        newGraph.csv_dialect = copy.deepcopy(self.csv_dialect)

        newGraph.graph_op_timing = copy.deepcopy(self.graph_op_timing)
        newGraph.graph_op_num_of_times = copy.deepcopy(self.graph_op_num_of_times)
        newGraph.graph_op_timing_recent = copy.deepcopy(self.graph_op_timing_recent)
        newGraph.graph_op_num_of_times_recent = copy.deepcopy(self.graph_op_num_of_times_recent)
        newGraph.last_timing_reset_time = copy.deepcopy(self.last_timing_reset_time)
        newGraph.graph_start_time = copy.deepcopy(self.graph_start_time)
        newGraph.pass_start_time = copy.deepcopy(self.pass_start_time)
        newGraph.solver_time = copy.deepcopy(self.solver_time)

        # optimizations
        newGraph.name_nodes = copy.deepcopy(self.name_nodes)
        newGraph.toplevel_file_nodes = copy.deepcopy(self.toplevel_file_nodes)
        newGraph.affected_name_nodes = copy.deepcopy(self.affected_name_nodes)

        # debug only
        newGraph.cg_paths = copy.deepcopy(self.cg_paths)
        newGraph.cf_paths = copy.deepcopy(self.cf_paths)
        newGraph.last_time = copy.deepcopy(self.last_time)
        newGraph.last_handled = copy.deepcopy(self.last_handled)
        newGraph.last_handled_lineno = copy.deepcopy(self.last_handled_lineno)

        # Cosseter Stuff
        newGraph.keepFullCode = copy.deepcopy(self.keepFullCode)


    def gregDebugCFGDebugHelp(self, ast_nodes):
        retLst = {}
        for ast_node in ast_nodes:
            retLst[ast_node] = []
            attr = self.get_node_attr(ast_node)
            namespace = attr.get("namespace", ast_node)
            retLst[ast_node].append([namespace, attr])
            objDefs = self.get_func_decl_objs_by_ast_node(ast_node)
            retLst[ast_node].append(objDefs)
            moreLst = {}
            for item in objDefs:
                attr = self.get_node_attr(item)
                name = attr.get("name", item)
                moreLst[name] = attr
            retLst[ast_node].append(moreLst)
        return retLst

    def gregDebugObjsDebugHelp(self, obj_nodes):
        retLst = {}
        for obj in obj_nodes:
            attr = self.get_node_attr(obj)
            retLst[obj] = attr
        return retLst
    ...
    def import_from_string(self, string):
        ...
        if self.exp:
            self.export_graph("/home/gttystah/GitHub_Permission_Reduction/abstractTests/graph")
            self.import_graph("/home/gttystah/GitHub_Permission_Reduction/abstractTests/graph")
            self.exp = False
    ...
    def import_graph(self, file_path):
        ...
        """
        self.graph = nx.readwrite.read_gpickle(file_path)
    ...
    def setup2(self):
        ...
        self.infinity_obj = self.add_obj_node(None, 'number', float('inf'))
    ...
    def get_class_obj_for_method(self, methodObj):
        return self.get_node_attr(methodObj).get("CLASS_OBJ_FOR_THIS", None)
