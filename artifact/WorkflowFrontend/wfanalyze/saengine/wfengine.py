...
class TaintEngine(object):
    ...
    def __init__(self, workflow : Workflow, metadata : dict, debug_mode = False, inputs = []):
        self.workflow = workflow
        self.debug = debug_mode
        
        self.id = metadata["id"]
        self.repo = metadata["repo"]
        self.wf_path = metadata["wf_path"]
        self.wf_name = metadata["wf_name"]
        self.wf_metadata = metadata.get("wf_metadata", {})
        self.reusable_wf_name = metadata.get("reusable_wf_name", "")
        self.wf_yaml = metadata.get("wf_yaml", "")
        self.ir = metadata.get("ir", {})
    ...
    def run(self):
        ...
        return self.tainted_workflow_outputs()

    def tainted_workflow_outputs(self): 
        tainted_outputs = []
        for output in self.workflow.workflow_outputs:
            if self.debug:
                self.print_str(f"Checking output : {output['name']} | workflow level")
            sources = self.contains_tainted_variable(output)
            if sources != []:
                obj = TaintObj(
                    name=output['name'],
                ...
                tainted_outputs.append(obj)
        return tainted_outputs

    def handle_reusable_workflow(self, task_group : TaskGroup):
        wf_name = task_group.tasks[0].name
        wf_inputs = task_group.tasks[0].inputs

        tainted_inputs = []
        for inp in wf_inputs:
            sources = self.contains_tainted_variable(inp)
            if sources != []:
                obj = TaintObj(
                    name=inp['name'],
                    type="wf_input",
                ...
                tainted_inputs.append(obj)

        if self.debug:
            self.print_str(f"wf_name: {wf_name}")

        if "@" in wf_name:
            wf_name = wf_name.split("@")[0]

        ir_workflow = None
        if wf_name.startswith("./"):
            if self.debug:
                self.print_str(f"Local workflow {wf_name}")
            ir_workflow = get_ir_workflow(self.repo, wf_name)
        else:
            repo, wf_path = split_action_name(wf_name)
            if self.debug:
                self.print_str(f"Reusable workflow {repo}:{wf_path}")
            ir_workflow = get_ir_workflow(repo, wf_path)
        
        if ir_workflow == None:
            if self.debug:
                self.print_str(f"Workflow not found in the database")
            return
        if self.debug:
            self.print_str(f"Workflow found in the database")

        # Pass the same metadata to the reusable workflow
        # maybe we need to cache it.  
        wf_outputs = TaintEngine(Workflow(ir_workflow['ir']), {
            "id" : self.id,
            "repo" : self.repo,
            "wf_path" : self.wf_path,
            "wf_name" : self.wf_name,
            "reusable_wf_name" : ir_workflow['repo'] + "@" + ir_workflow['workflow_path'],
        }, self.debug, tainted_inputs).run()

        for output in wf_outputs:
            self.taint_job_output(output)
...
# 
    ...
    def check_sinks(self, task : Task, results, name):
        ...
        ArgToSink = results["ArgToLSink"]
        for arg in ArgToSink:
            taint_arg = self.is_arg_tainted(arg["name"])
            if taint_arg:
                self.raise_alert(
                    atype=self.ARG_TO_LSINK, 
                    alert=f"{arg['name']}",
                    details={
                        "action": task.exec_command,
                        "arg": arg["name"],
        ...
        EnvToSink = results.get("EnvtoLSink", [])
        for env in EnvToSink:
            taint_env = self.is_env_tainted(env["name"])
            if taint_env:
                self.raise_alert(
                    atype=self.ENV_TO_LSINK, 
                    alert=f"{env['name']}",
                    details={
                        "action": task.exec_command,
                        "env": env["name"],
        ...
        ContextToSink = results["ContextToSink"]
        for arg in ContextToSink:
            taint_arg = TaintObj(
                name=arg["name"],
                type="js_context",
                parent_nodes=[],
            ...
            self.raise_alert(
                atype=self.CONTEXT_TO_SINK, 
                alert=f"context {arg['name']}",
                details={
                    "action": task.exec_command,
                    "arg": arg["name"],
                    "ArgToSink": arg,
    ...
    def propogate_taint(self, task : Task, results):
        ...
        ContextToOutput = results.get("ContextToOutput", [])
        for context in ContextToOutput:
            for argout in context["output"]:
                if argout["function"] == "setOutput":
                    obj = TaintObj(
                        name=argout["name"],
                        type="jscontext_output",
                        parent_nodes=[],
                        location=self.get_location()  
                    )
                    self.taint_output(obj)
